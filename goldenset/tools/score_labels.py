...
이 스크립트가 내는 숫자
  ...
  2. **clean 일치율** — 조원 2인만의 일치율. 출제자 오염이 없어
     judge 일치율의 *천장*으로 쓴다 (judge는 사람보다 정확할 수 없다)
...
def render(sheets: list[Sheet], pairs: list[dict], clean: dict | None) -> str:
    ...
    if clean:
        L += ["", "## 2. clean 일치율 — judge 일치율의 천장", ""]
        L += [f"조원 2인이 공통으로 본 **{clean['n']}건** 기준. 출제자 오염 없음.", ""]
        L += [f"### **{clean['rate']}**  ({clean['pair']})", ""]
        L += [
            "judge 일치율은 이 값을 넘을 수 없다. 사람끼리도 갈리는 폭보다",
            "정밀하게 잴 수 없기 때문이다. judge 숫자는 반드시 이 값과 함께 제시한다.",
            "",
        ]
    else:
