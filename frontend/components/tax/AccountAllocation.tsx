...
const ACCOUNT_HELP = (isaLimitManwon: number) => [
  ...
  "미사용분은 해마다 누적: 리셋 없음",
  `총 납입한도 1억원: 5년차 도달 후 증가 없음${
    isaLimitManwon >= 10000 ? " (이 고객이 그 자리)" : ""
  }`,
  "인출해도 한도 미복원: 넣었다 뺀 금액도 누적에 포함",
  "연금계좌는 연 900만원 세액공제 한도: 매년 1월 1일 리셋 (소득세법 §59의3)",
  "막대는 기납입액: 올해 배분안은 납입안 탭",
  "전체 계좌 띠는 확정 대상 안의 자산 구성: 제안 조정 시 함께 변동",
];
    ...
    <div className="flex w-full flex-1 flex-col">
      ...
      */}
      ...
      {/*
        ...
        content-evenly — 박스가 늘어나면 남는 높이를 범례→ISA→연금→각주 간격에
        고르게 나눈다. 한 곳에 몰리면 막대가 떨어져 나간 것처럼 보인다.
      */}
      <div className="grid flex-1 grid-cols-[82px_1fr_auto] content-evenly items-center gap-x-2 gap-y-4">
        {bars.map((bar) => {
          const pct =
