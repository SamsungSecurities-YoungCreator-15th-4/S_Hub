...
def test_var_engine_respects_lookback_config(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "engine.nodes.var_engine.load_returns", _load_returns_with_tmp_cache(tmp_path)
    )
    state = {
        "run_config": {
...
def test_var_engine_uses_real_data_by_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(returns_mod, "_fetch_real_returns", _fake_fetch_real_returns)
    monkeypatch.setattr("engine.nodes.var_engine.load_real_returns", _load_real_returns_with_tmp_cache(tmp_path))
    state = {
...
def test_var_engine_preserves_rf_rate_zero(monkeypatch):
    ...
    monkeypatch.setattr("engine.nodes.var_engine.load_real_returns", fake_load_real_returns)
    state = {
