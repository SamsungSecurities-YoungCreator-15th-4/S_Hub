...
from engine.llm.audit import prompt_hash_record, with_llm_audit
from engine.graph import build_graph
from engine.nodes.assemble_report import assemble_report
from engine.nodes.judge_eval import _AuditedLLM, _named_prompts, judge_eval
from engine.nodes.load_inputs import load_inputs
from engine.observability.langsmith import (
...
def test_enabled_invocation_connects_trace_id_and_root_run(monkeypatch):
    ...
    monkeypatch.setattr("engine.observability.langsmith.uuid.uuid4", lambda: fixed_run_id)
    monkeypatch.setattr(
        "engine.observability.langsmith._get_run_url",
...
def test_judge_failure_adds_trace_metadata_and_report_audit(monkeypatch):
    ...
    monkeypatch.setattr("engine.nodes.judge_eval.annotate_current_run", capture_annotation)
    state = _judge_state()
