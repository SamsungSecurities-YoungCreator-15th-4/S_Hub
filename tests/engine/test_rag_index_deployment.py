...
def test_blob_artifact_is_downloaded_validated_installed_and_cached(
    ...
    monkeypatch.setattr("engine.rag.deployment._download_bytes", fake_download_bytes)
    monkeypatch.setattr("engine.rag.deployment._download_file", fake_download_file)
    settings = IndexSupplySettings(
...
def test_corrupted_download_is_removed_from_cache_immediately(
    ...
    monkeypatch.setattr(
        "engine.rag.deployment._download_bytes",
        lambda _url, *, limit: manifest_path.read_bytes(),
    )
    monkeypatch.setattr(
        "engine.rag.deployment._download_file",
