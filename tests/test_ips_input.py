...
from engine.llm.extract_ips_chain import extract_ips_profile
from engine.nodes.conflict_check import conflict_check
from engine.nodes.extract_ips import extract_ips
from engine.nodes.load_inputs import (
    ASSET_DEFINITIONS,
    SAMPLE_RAW_INPUT,
    load_inputs,
    portfolio_from_percentages,
)
from engine.state import FIXED_GOAL, FIXED_JOB, IPSProfile, UNIQUE_PREFIX
...
def test_extract_node_offline_mode_does_not_require_llm(monkeypatch):
    ...
    monkeypatch.setattr(
        "engine.nodes.extract_ips.extract_ips_profile_with_meta", fail_if_called
    )
    result = extract_ips(
...
def test_conflict_policy_is_cached_after_first_load():
    import engine.nodes.conflict_check as conflict_module
...
def test_load_inputs_preserves_ui_raw_input_and_portfolio(monkeypatch, tmp_path):
    ...
    import engine.nodes.load_inputs as load_inputs_module
